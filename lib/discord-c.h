#ifndef INC__DISCORD_C_H
#define INC__DISCORD_C_H
#include "websocket.h"
#include "stdio.h"
#include "stdlib.h"
#include "string.h"
...
/* Sends a message with ”content” as a body to the channel with the “channel”
 * ID. Makes the message say out loud if isTTS is true (and the server doesn’t
 * ban it) */
...
/* Stores the information associated with a connection */
struct connection
{
...
/* Struct to store chunks of messages. Used so the message struct’s next will
 * be reliable */
...
typedef void (*discord_message_posted_callback)(struct message message);
...
struct messages *getMessagesInChannel(uint64_t channel, int ammount);
...
/* Loads a guild into memory (fetches users, presences and starts to recieve
 * events such as precense update etc) */
void loadGuild(client_websocket_t *socket, uint64_t guildId);

#endif /* INC__DISCORD_C_H */
